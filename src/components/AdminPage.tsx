    ...
    try {
      const res = await fetch('/api/admin', {
        ...
        body: JSON.stringify({
          ...
          agentMode: newPromoPlan !== 'BASIC',
          advancedAI: newPromoPlan === 'PREMIUM' || newPromoPlan === 'LIFETIME',
          unlimitedChat: newPromoPlan === 'PREMIUM' || newPromoPlan === 'LIFETIME',
          priority: newPromoPlan === 'PREMIUM' || newPromoPlan === 'LIFETIME',
          exportPDF: newPromoPlan === 'PREMIUM' || newPromoPlan === 'LIFETIME',
          ocrUnlimited: newPromoPlan === 'LIFETIME',
          customPlans: newPromoPlan === 'LIFETIME'
        }),
        credentials: 'include'
      })
      const data = await res.json()
    ...
    >
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="stats">
          {loading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <StatCard label="المستخدمين النشطين" value={stats.activeUsers} icon={<CheckCircle />} color="green" />
              <StatCard label="المستخدمين المميزين" value={stats.premiumUsers} icon={<Crown />} color="purple" />
              <StatCard label="المستخدمين المحظورين" value={stats.bannedUsers} icon={<Ban />} color="red" />
              <StatCard label="إجمالي المحادثات" value={stats.totalChats} icon={<FileText />} color="blue" />
              <StatCard label="الاشتراكات النشطة" value={stats.activeSubscriptions} icon={<Ticket />} color="green" />
              <StatCard label="التسجيلات الأخيرة" value={stats.recentSignups} icon={<Users />} color="orange" />
        ...
        <TabsContent value="users">
          <Card className="p-4">
            <div className="flex gap-4 mb-4">
              ...
              />
              <Button onClick={fetchUsers}>بحث</Button>
            </div>
        ...
        <TabsContent value="promocodes">
          <Card className="p-4">
            <h3 className="font-semibold mb-4">إنشاء كود جديد</h3>
            ...
            <Button onClick={createPromoCode} disabled={promoLoading || !newPromoCode} className="mb-6">
              {promoLoading ? (
            ...
            <h3 className="font-semibold mb-4">الأكواد الموجودة</h3>

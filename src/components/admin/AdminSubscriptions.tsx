...
export default function AdminSubscriptions({ showMessage }: AdminSubscriptionsProps) {
  ...
  const fetchSubscriptions = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin?action=subscriptions&${params}`, { 
        credentials: 'include' 
      })
  ...
  const expiringCount = subscriptions.filter(s => isExpiringSoon(s.endDate)).length
  const expiredCount = subscriptions.filter(s => s.status === 'EXPIRED').length
  const premiumCount = subscriptions.filter(s => s.plan === 'PREMIUM' || s.plan === 'BAC_PRO').length
    ...
    <div className="space-y-4">
      ...
      {error && (
        <Card className="p-4 bg-red-50 dark:bg-red-900/20 border-red-200">
          ...
          <Button onClick={fetchSubscriptions} variant="outline" size="sm" className="mt-2 w-full">
            إعادة المحاولة
          </Button>
        </Card>
      )}
          ...
          <div className="space-y-2 max-h-[400px] overflow-y-auto">
            {subscriptions.map((sub) => {
              ...
              return (
                <div
                  key={sub.id}
                  className={`flex items-center justify-between p-3 rounded-xl border gap-2 ${expiring ? 'border-orange-300 bg-orange-50 dark:bg-orange-900/10' : ''}`}
                >

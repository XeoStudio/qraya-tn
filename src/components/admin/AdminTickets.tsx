            ...
            <div className="space-y-2 max-h-[500px] overflow-y-auto">
              {tickets.map((ticket) => (
                ...
                >
                  ...
                  <p className="text-xs text-gray-500 truncate">{ticket.user?.name || ticket.user?.email}</p>
                  <div className="flex items-center gap-1 mt-1 flex-wrap">
                    ...
                    {ticket.needsHumanIntervention && (
                      <Badge variant="destructive" className="text-xs"><AlertTriangle className="w-3 h-3 ml-1" />تدخل</Badge>
                    )}
                  </div>
                </button>
              ))}
              ...
              <div className="pb-3 border-b mb-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{selectedTicket.user?.name || selectedTicket.user?.email}</p>
                  </div>
                  {selectedTicket.status !== 'CLOSED' && (
                    <Button size="sm" variant="outline" onClick={closeTicket}>إغلاق</Button>
                  )}
                </div>
                ...
                {selectedTicket.messages?.map((msg) => (
                  <div key={msg.id} className={`flex gap-2 ${msg.isFromAdmin ? '' : msg.isFromAI ? '' : 'flex-row-reverse'}`}>
                    ...
                    <div className={`rounded-xl px-3 py-2 max-w-[85%] ${
                      ...
                      <p className="text-xs opacity-60 mt-1">{new Date(msg.createdAt).toLocaleString('ar')}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {selectedTicket.status !== 'CLOSED' && (
                <div className="pt-3 border-t mt-3">
                  <div className="flex gap-2">
                    <Textarea value={replyMessage} onChange={(e) => setReplyMessage(e.target.value)} placeholder="اكتب ردك..." rows={2} className="flex-1 resize-none" />
